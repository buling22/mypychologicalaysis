...
1. **职业层面（标签1）** 文本主题最集中，work/job 是六类中 TF-IDF 得分最高的特征词，模型分类效果较好（f1=0.63）
2. **身体层面（标签3）** 医学词汇密集（ptsd、antidepressants、diagnosed），词汇丰富度最高（0.38），模型分类效果最佳（f1=0.72）
3. **精神层面（标签2）** 文本最短（20.86词）、词汇丰富度最低（0.29），却包含最极端的词汇（suicide、death），话语的匮乏本身就是一种信号
4. **社交层面（标签4）** 样本量最多（405条），family、friends、lonely、alone 同时高频，孤独感是这类文本最鲜明的底色
5. **认知（标签0）、精神（标签2）、情感（标签5）** 词汇高度重叠，TF-IDF 层面难以区分，混淆矩阵中这三类误分类最集中，EDA 的判断得到了建模验证
6. 纯词频/TF-IDF 特征对语义相近类别的区分能力有限，后续引入 BERT 等语义模型可进一步提升分类精度
